...
#             kernel = np.ones((17, 17), np.uint8)# change kernel size
#
...
# current_velocity_sub = rospy.Subscriber(name="/current_linear_x", data_class=Float64, callback=currentVelCallback)
############parameter###############
# short roi
np.Fi_lay_R = [630, 472]
np.S_lay_R = [500, 334]
...
# edge_tracking()np.Fi_lay_R=[630, 472]
# np.S_lay_R=[500, 334]
#
# np.Fi_lay_L=[9, 472]
# np.S_lay_L=[139, 334]
...
# cap= cv2.VideoCapture("/home/ubuntu/contest/video/output05_03_07_28.avi")
########video save###################
        ...
        if frame_num % 5 == 0:
            ...
            for i in range(len(out_l_keypoint_coords)):
                ...
                for j in range(len(out_L_middle_line)):
                    L_out_wb_distance.append(math.sqrt((out_l_keypoint_coords[i][0] - out_L_middle_line[j][0]) ** 2 + (
                                out_l_keypoint_coords[i][1] - out_L_middle_line[j][1]) ** 2))
            ...
            for i in range(len(out_r_keypoint_coords)):
                ...
                for j in range(len(out_R_middle_line)):
                    R_out_wb_distance.append(math.sqrt((out_r_keypoint_coords[i][0] - out_R_middle_line[j][0]) ** 2 + (
                                out_r_keypoint_coords[i][1] - out_R_middle_line[j][1]) ** 2))
            ...
            for i in range(len(L_keypoint_coords)):
                ...
                for j in range(len(L_middle_line)):
                    L_wb_distance.append(math.sqrt((L_keypoint_coords[i][0] - L_middle_line[j][0]) ** 2 + (
                                L_keypoint_coords[i][1] - L_middle_line[j][1]) ** 2))
            ...
            for i in range(len(R_keypoint_coords)):
                ...
                for j in range(len(R_middle_line)):
                    R_wb_distance.append(math.sqrt((R_keypoint_coords[i][0] - R_middle_line[j][0]) ** 2 + (
                                R_keypoint_coords[i][1] - R_middle_line[j][1]) ** 2))
            ...
            for i in range(len(T_keypoint_coords)):
                ...
                for j in range(len(T_middle_line)):
                    T_wb_distance.append(math.sqrt((T_keypoint_coords[i][0] - T_middle_line[j][0]) ** 2 + (
                                T_keypoint_coords[i][1] - T_middle_line[j][1]) ** 2))
            ...
            if len(T_line) == 1 and len(out_L_line) != 0 and T_line[0][0][0] > 150:
                if Right_mean_ang == 0:
                    ...
                    # if current_velocity < 0.40:
                #     drive_data.linear.x = 200
                # elif current_velocity > 1.3:
                #     drive_data.linear.x = 0
                # else:
                #     drive_data.linear.x = speed
                drive_data.linear.x = speed
                drive_data.angular.z = 50  # -50
                ...
                # drive_data.linear.y = 1
                drive_data.linear.x = speed
                drive_data.angular.z = (-1) * mean_ang * weight
            #################확 꺾일 때 예외처리#######################
            # if mean_ang >= 0:
...
df = pd.DataFrame(total_list,
                  columns=['left_ang', 'right_ang', 'len_left', 'len_right', 'mean_ang', 'linear_x', 'ang_z',
                           'len(T_line)', 'len(out_L_line)', 'len(out_R_line)'])
