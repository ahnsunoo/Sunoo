...
int main (int argc, char** argv)
{
    ...
    while(ros::ok())
    {
		...
		if (serial_fd >= 0) 
		{
			...
			imu_data_msg.header.frame_id = tf_prefix_ + "/imu_link";  // "imu_link"   , 원래 "/imu_link"였는데 "imu_link"로 바꿈
			...
			if(m_bSingle_TF_option)
			{
				...
				//br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "base_link", "imu_link")); // 수정후 이걸 주석 해제
				br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), tf_prefix_ + "/base_link", tf_prefix_ + "/imu_link")); //수정 전 코드는 이걸 썼음
